  ...
  const handleUpdate = (isNew=false) => {
    ...
    if (isNew) {
      ...
      newRecords = records.map((r) =>
        r.id === editingId ? { ...r, hotkey, contentList: filterContentList, sendDelay } : r
      )
    }
  ...
  return (
    <div className="p-3">
      ...
      <div>
        ...
        <div>
        <Button onClick={() => handleUpdate(true)} size="small" variant="dark">
          插入
        </Button>
        </div>
        <div>
        {isUpdating && (
          <Button onClick={() => handleUpdate()} size="small" variant="default" className="ml-3">
            更新
          </Button>
        )}
        {isUpdating && (
        ...
        <Button onClick={handleReset} size="small" variant="default" className="ml-3">
          重置
          </Button>
